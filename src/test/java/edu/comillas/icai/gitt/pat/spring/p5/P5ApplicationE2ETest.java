...
class P5ApplicationE2ETest {
    ...
    @Test public void loginOkTest() {
        // Given: primero registramos al usuario
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        String registro = "{" +
                "\"name\":\"" + NAME + "\"," +
        ...
        // Then: comprobamos que se devuelve un token
        Assertions.assertEquals(HttpStatus.OK, response.getStatusCode());
        String token = response.getBody();
        Assertions.assertNotNull(token);
        Assertions.assertTrue(token.length() > 10); // asumiendo que el token tiene cierta longitud
    }
}
